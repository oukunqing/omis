...
namespace OMIS.DAL
{
    public abstract class MysqlDataAccess : OMIS.DBA.MysqlDataAccess
    {
        ...
        public static int GetMaxId(string conn, string tableName, string fieldName)
        {
            try
            {
                return Convert.ToInt32(MysqlHelper.ExecuteScalar(conn, String.Format("select ifnull(max(`{0}`),0) from `{1}`;", fieldName, tableName)));
            }
        ...
        public static int GetTypeLevel(string conn, string levelFieldName, string tableName, string fieldName, int fieldValue)
        {
            try
            {
                return Convert.ToInt32(MysqlHelper.ExecuteScalar(conn, String.Format("select ifnull(`{0}`,-1) as id,count(`{1}`) from `{2}` dt where `{3}` = {4};", levelFieldName, fieldName, tableName, fieldName, fieldValue)).ToString());
            }
            catch (Exception ex) { throw (ex); }
        }
        #endregion
        ...
        public static bool CheckTableExist(string conn, string dbName, string tableName)
        {
            string sql = String.Format("select count(*) as exist from information_schema.tables where table_schema = '{0}' and table_name = '{1}';", dbName, tableName);
        ...
        public static int UpdateParentTree(string conn, string tableName, string fieldName, int fieldValue, int minLevel, string levelFieldName)
        {
            try
            {
                ...
                return MysqlHelper.ExecuteNonQuery(conn, sql.ToString());
            }
            catch (Exception ex) { throw (ex); }
        }
        #endregion
    ...
    #region  MyDBA
    public abstract class MyDBA : MysqlDataAccess
    {
    }
    #endregion
