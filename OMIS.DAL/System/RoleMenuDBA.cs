...
{
    public class RoleMenuDBA : DataAccess
    {

        #region  获得单个角色-导航菜单
        public DataResult GetRoleMenu(int id)
        {
            try
            {
                StringBuilder sql = new StringBuilder();
                sql.Append(" select d.* from `sys_role_menu` d ");
        ...
        #region  获得多个角色-导航菜单
        public DataResult GetRoleMenu(string idList)
        {
            try
            {
        ...
        #region  获得角色-导航菜单
        public DataResult GetRoleMenu(Dictionary<string, object> dic)
        {
            try
            {
                #region  Condition
        ...
        #region 新增角色-导航菜单
        public DataResult AddRoleMenu(RoleMenuInfo o)
        {
            try
            {
                StringBuilder sql = new StringBuilder();
                ...
                result.Result = result.Result > 0 ? GetMaxId(DBConnString, "sys_role_menu", "id") : 0;
        ...
        #region 更新角色-导航菜单
        public DataResult UpdateRoleMenu(RoleMenuInfo o)
        {
            try
            {
                StringBuilder sql = new StringBuilder();
                sql.Append(" update `sys_role_menu` set ");
        ...
        #region  获得角色-导航菜单配置
        public DataResult GetRoleMenuConfig(Dictionary<string, object> dic)
        {
            try
            {
                ...
                if (roleId > 0)
                {
                    ...
                    sql.Append(";");
                    //菜单分类
                    sql.Append(" select d.type_id,d.type_name from `sys_menu_type` d ;");
                    //菜单
                    sql.Append(" select d.menu_id,d.menu_name,d.menu_code,d.type_id,d.enabled from `sys_menu` d; ");
                    //角色-菜单
                    sql.Append(" select r.role_id,d.menu_id from sys_role_menu rm,sys_role r,sys_menu d ");
        ...
        #region  批量新增角色-导航菜单
        public DataResult BatchAddRoleMenu(List<RoleMenuInfo> list)
        {
            try
            {
                if (list.Count > 0)
                {
                    ...
                    int n = 0;
                    foreach (RoleMenuInfo o in list)
                    {
                        sql.Append(n++ > 0 ? "," : "");
                        sql.Append("(");
        ...
        public DataResult BatchAddRoleMenu(int roleId, string menuIdList, int operatorId, string createTime)
        {
            try
            {
        ...
        #region 删除角色-导航菜单
        public DataResult DeleteRoleMenu(int id)
        {
            try
            {
                string sql = String.Format("delete from `sys_role_menu` where `id` = {0};", id);
        ...
        public DataResult DeleteRoleMenu(int roleId, int menuId)
        {
            try
            {
        ...
        #region  填充数据
        public RoleMenuInfo FillRoleMenuInfo(DataRow dr)
        {
            try
            {
                RoleMenuInfo o = new RoleMenuInfo();
        ...
        public RoleMenuInfo FillRoleMenuInfo(DataRowView drv)
        {
            try
            {
                return this.FillRoleMenuInfo(drv.Row);
            }
            catch (Exception ex) { throw (ex); }
        }
        #endregion
