...
namespace OMIS.Server.Device
{
    class DeviceService
    {
        ...
        public DeviceService(TcpClient client)
        {
            ...
            Program.PrintConsoleLog(MsgLog);
        ...
        private void ReceiveClientMessage()
        {
            ...
            {
                Program.PrintConsoleLog(ex.Message);
            ...
            while (DevEnabled)
            {
                try
                {
                    ...
                    if (rcvData > 0)
                    {
                        ...
                        Program.PrintConsoleLog("收到报文:" + rcvMsg);
                ...
                catch (System.Exception ex)
                {
                    ...
                    if (DevClient == null || !DevClient.Client.Connected)
                    {
                        ...
                        Program.PrintConsoleLog(DevCode + ":设备掉线，断开设备连接，释放资源");
        ...
        protected void SendMessage(string rspMsg)
        {
            try
            {
                ...
                {
                    Program.PrintConsoleLog(String.Format("向设备[{0}]发送消息:{1}", DevCode, rspMsg));
