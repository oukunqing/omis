...
function getDataList(isReload) {
    ...
    var urlparam = 'action=getSensorOriginalTypeList&data=' + encodeURIComponent(module.toJsonString(param));
...
function showDataHead() {
    ...
    rd.push({ html: '分类名称', style: [['minWidth', '150px']] });
    rd.push({ html: '分类编码', style: [['minWidth', '120px']] });
    rd.push({ html: '操作', style: [['minWidth', '70px']] });
    rd.push({ html: '默认单位', style: [['minWidth', '60px']] });
...
function showDataList(jsondata, param) {
    ...
    for (var i = 0, c = jsondata.list.length; i < c; i++) {
        ...
        row.lang = dr.OriTypeId;

        row.ondblclick = function (e, i) {
            editData(this.lang);
        };
        var oper = [
            '<a onclick="editData(%s);">%s</a>'.format([dr.OriTypeId, page.lang["edit"]]),
            '<a onclick="deleteData(%s,\'%s\');">%s</a>'.format([dr.OriTypeId, dr.OriTypeName, page.lang["delete"]])
        ];

        rd.push({ html: rnum, style: [] });
        rd.push({ html: dr.OriTypeName, style: [] });
        rd.push({ html: dr.OriTypeCode, style: [] });
        rd.push({ html: oper.join('|'), style: [] });
        rd.push({ html: dr.DefaultUnit, style: [] });
        rd.push({ html: page.parseEnabled(dr.Enabled), style: [] });
        rd.push({ html: dr.SortOrder, style: [] });
        rd.push({ html: dr.CreateTime, style: [] });
        rd.push({ html: dr.OriTypeId, style: [] });
...
function editData(id) {
    ...
    var url = 'sensorOriginalTypeEdit.aspx?typeId=%s'.format([id]);
...
function deleteData(id, name) {
    ...
    page.deleteConfirm(function () {
        var urlparam = 'action=deleteSensorOriginalType&id=' + id;
        module.ajaxRequest({
            url: webConfig.webDir + '/ajax/sensor/sensor.aspx',
            data: urlparam, dataType: 'json',
            callback: function (data, param) {
