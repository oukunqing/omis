...
public partial class tools_dba : System.Web.UI.Page
{
    ...
    public void BuildFieldModel(DataSet ds, string className, string classDesc)
    {
        try
        {
            ...
            StringBuilder strGetLevel = new StringBuilder();
            StringBuilder strUpdateParentTree = new StringBuilder();

            StringBuilder strCheckExist = new StringBuilder();
            ...
            */
            ...
            foreach (DataRow dr in ds.Tables[0].Rows)
            {
                ...
                else if (field.EndsWith("name") && !hasName)
                {
                    hasName = true;
                    nameField = field;
                }
                else if (field.EndsWith("code") && !hasCode)
                {
                    hasCode = true;
                    codeField = field;
                }
                else if (field.EndsWith("number") && !hasNumber)
                {
                    hasNumber = true;
                    numField = field;
                }
                else if (field.EndsWith("mobile") && !hasMobile)
                {
                    hasMobile = true;
                    mobileField = field;
                }
                else if (field.EndsWith("email") && !hasEmail)
                {
                    hasEmail = true;
                    emailField = field;
                }
                ...
                else if (field.Equals("parent_tree"))
                {
                    hasParentTree = true;
                }
                #endregion
            ...
            #region  排序
            string strOrderBy = "";
            if (hasSortOrder || hasPriKey)
            {
                string _ob = "sql.Append(\" order by {0}{1} \");\r\n ";
                string _ob1 = "";
                string _ob2 = "";
                switch (DBTYPE)
                {
                    case Mysql:
                        _ob1 = hasSortOrder ? "d.`sort_order` desc" : "";
                        _ob2 = hasPriKey ? String.Format("d.`{0}`", priKey.field) : "";
                        break;
                    case Mssql:
                        _ob1 = hasSortOrder ? "d.[sort_order] desc" : "";
                        _ob2 = hasPriKey ? String.Format("d.[{0}]", priKey.field) : "";
                ...
                if (_ob1.Length > 0 && _ob2.Length > 0)
                {
                    _ob2 = "," + _ob2;
                }
                strOrderBy = String.Format(_ob, _ob1, _ob2);
            }
            #endregion
            ...
            if (hasEnabled)
            {
                strGet3.Append("int enabled = ConvertValue(dic, \"Enabled\", -1);\r\n");
                strGet3.Append(String.Format("con.Append(enabled >= 0 ? String.Format(\" and d.{0}enabled{1} = {{0}} \", enabled) : \"\");\r\n\r\n", L, R));
            }
            if (hasParent)
            {
                strGet3.Append("int parentId = ConvertValue(dic, \"ParentId\", 0);\r\n");
                strGet3.Append("if (parentId > 0)\r\n");
                strGet3.Append("{\r\n");
                strGet3.Append("bool isGetSubset = ConvertValue(dic, \"GetSubset\", 0) == 1;\r\n");
                strGet3.Append(String.Format("con.Append(String.Format(isGetSubset ? \" and d.parent_tree like '%({{0}})%' \" : \" and d.{0}parent_id{1} = {{0}} \", parentId));\r\n", L, R));
                strGet3.Append("}\r\n");

                if (hasParentTree)
                {
                    strGet3.Append("int excludeId = ConvertValue(dic, \"ExcludeId\", 0);\r\n");
                    strGet3.Append("if (excludeId > 0)\r\n");
                    strGet3.Append("{\r\n");
                    strGet3.Append(String.Format("con.Append(String.Format(\" and d.{0}parent_tree{1} not like '%({{0}})%' \", excludeId));\r\n", L, R));
                    strGet3.Append("}\r\n");
                }
            }
            strGet3.Append("\r\nstring keywords = ConvertValue(dic, \"Keywords\");\r\n");
            strGet3.Append("if (!keywords.Equals(string.Empty))\r\n");
            strGet3.Append("{\r\n");
            strGet3.Append("string searchField = ConvertValue(dic, \"SearchField\");\r\n");
            strGet3.Append("switch (searchField)\r\n");
            strGet3.Append("{\r\n");
            strGet3.Append("case \"Id\":\r\n");
            strGet3.Append(String.Format("con.Append(CheckIdList(ref keywords) ? String.Format(\" and d.{0}{2}{1} in ({{0}}) \", keywords) : \"\");\r\n", L, R, priKey.field));
            strGet3.Append("break;\r\n");
            if (hasName)
            {
                strGet3.Append("case \"Name\":\r\n");
                strGet3.Append(String.Format("con.Append(DBC.BuildSearchCondition(keywords, \" and d.{0}{2}{1} like '%{{0}}%' \"));\r\n", L, R, nameField));
                strGet3.Append("break;\r\n");
            }
            strGet3.Append("}\r\n");
            strGet3.Append("}\r\n");
            ...
            strGet3.Append(String.Format("sql.Append(\" select d.* from {0}{2}{1} d \");\r\n", L, R, tableName));
            strGet3.Append("sql.Append(\" where 1 = 1 \");\r\n");
            strGet3.Append("sql.Append(con.ToString());\r\n");
            strGet3.Append(strOrderBy);
            ...
            {
                strGet3.Append("\r\n");
                strGet3.Append(String.Format("sql.Append(\" select count(distinct d.{0}{2}{1}) as dataCount from {0}{3}{1} d \");\r\n", L, R, priKey.field, tableName));
            ...
            string[,] checkField = new string[,] {
                {nameField,"Name_名称"},{codeField,"Code_编码"},{numField,"Number_编号"},{mobileField,"Mobile_手机号码"},{emailField,"Email_Email地址"}
            };
            for (int i = 0; i < checkField.Length / 2; i++)
            {
                if (!checkField[i,0].Equals(string.Empty))
                {
                    string _fn = ParseFieldName(checkField[i, 0], true);
                    string _kn = ParseFieldName(priKey.field, true);
                    string[] _rn = checkField[i, 1].Split('_');

                    strCheckExist.Append(String.Format("\r\n#region  检测{0}是否存在\r\n", _rn[1]));
                    strCheckExist.Append(String.Format("public bool Check{0}IsExist(string {1}, int {2})\r\n", _rn[0], _fn, _kn));
                    strCheckExist.Append("{\r\n");
                    strCheckExist.Append("try{\r\n");
                    strCheckExist.Append(String.Format("return CheckDataIsExist(DBConnString, \"{0}\", \"{1}\", \"{2}\", {3}, {4});", tableName, priKey.field, checkField[i, 0], _fn, _kn));
                    strCheckExist.Append("}\r\ncatch (Exception ex) { throw (ex); }\r\n");
                    strCheckExist.Append("}\r\n");
                    strCheckExist.Append("#endregion\r\n");
                }
            }
            ...
            #region  获得Level
            strGetLevel.Append("#region  获得层级\r\n");
            strGetLevel.Append(String.Format("public int GetLevel(int {0})\r\n", idKey));
            strGetLevel.Append("{\r\n");
            strGetLevel.Append(String.Format("return GetTypeLevel(" + dbconn1 + ", \"level\", \"{0}\", \"{1}\", {2});\r\n", tableName, priKey.field, idKey));
            strGetLevel.Append("}\r\n");
            strGetLevel.Append("#endregion\r\n");
            #endregion
            ...
            sqlA.Append(String.Format("sql.Append(\"insert into {0}{2}{1}(\");\r\n",L, R, tableName));
            ...
            sqlU.Append(String.Format("sql.Append(\" update {0}{2}{1} set \");\r\n", L,R, tableName));
            ...
            strUpdateParentTree.Append(String.Format("#region  更新{0}目录树\r\n", classDesc));
            strUpdateParentTree.Append(String.Format("public int UpdateParentTree(int {0})\r\n", idKey));
            strUpdateParentTree.Append("{\r\n");
            strUpdateParentTree.Append(String.Format("return UpdateParentTree(" + dbconn1 + ", \"{0}\", \"{1}\", {2}, 0);\r\n", tableName, priKey.field, idKey));
            strUpdateParentTree.Append("}\r\n");

            strUpdateParentTree.Append(String.Format("public int UpdateParentTree(int {0}, int minLevel)\r\n", idKey));
            strUpdateParentTree.Append("{\r\n");
            strUpdateParentTree.Append(String.Format("return UpdateParentTree(" + dbconn1 + ", \"{0}\", \"{1}\", {2}, minLevel);\r\n", tableName, priKey.field, idKey));
            strUpdateParentTree.Append("}\r\n");
            strUpdateParentTree.Append("#endregion\r\n");
            #endregion
            ...
            this.txtGetLevel.Text = strGetLevel.ToString();

            this.txtAddMethod.Text = sqlAdd1.ToString();
            this.txtUpdateMethod.Text = sqlUpdate1.ToString();
            this.txtDelMethod.Text = sqlDel1.ToString();
            this.txtUpdateParentTree.Text = strUpdateParentTree.ToString();

            this.txtCheckData.Text = strCheckExist.ToString();
            ...
            this.txtDataOperateDAL.Text = this.BuildResult(new List<string>() { 
                ...
                this.txtAddMethod.Text, this.txtUpdateMethod.Text, this.txtDelMethod.Text,
                this.txtFillData.Text
            }); 
    ...
    public string ParseFieldName(string fieldName, bool firstCharLower)
    {
        if (fieldName.Equals(string.Empty))
        {
            return string.Empty;
        ...
        foreach (string str in arrName)
        {
            if (firstCharLower && n == 0)
            {
                name.Append(str.Substring(0, 1).ToLower());
            }
            else
            {
                name.Append(str.Substring(0, 1).ToUpper());
    ...
    {
        if (type.IndexOf("int") >= 0 || type == "float" || type == "double" || type == "decimal")
        {
            return "0";
        }
        else
    ...
    public string ConvertData(string type, string field, string convertFunc)
    {
        ...
        {
            return String.Format("{0}(dr[\"{1}\"], 0.0d)", convertFunc, field);
        }
        else if (type == "decimal")
        {
            return String.Format("{0}(dr[\"{1}\"], 0.0m)", convertFunc, field);
        }
        else
        {
            if (this.chbConvertTime.Checked && field.EndsWith("_time"))
            {
                convertFunc = this.txtConvertTime.Text.Trim();
                return String.Format("{0}(dr[\"{1}\"].ToString())", convertFunc, field);
            }
    ...
    protected void chbConvertTime_CheckedChanged(object sender, EventArgs e)
    {
        string className = this.txtClassName.Text.Trim();
        string classDesc = this.txtClassComment.Text.Trim();
        this.BuildFieldModel(dsField, className, classDesc);
